...
qplot(stepsEachDay, geom="histogram") + geom_histogram(aes(fill = ..count..)) +
    labs(x="Total steps taken each day", y="Frequency count") +
    guides(fill=FALSE) + ggtitle("Frequency of total steps taken each day") +
...
averageActivityInterval<-as.data.frame(cbind(unique(activity$interval),
    averageActivity))
...
ggplot(data=averageActivityInterval,aes(x=interval, y=averageSteps, color="")) + 
    ...
    ggtitle("Average number of steps at each 5-min interval") +
    theme(plot.title = element_text(lineheight=0.8, face="bold"))
```
...
The maximum average number of steps across all days is: **`r maxInterval`**
...
```{r message=FALSE, warning=FALSE}
# Replace NA values with mean for the average number of steps at 
# each 5-min interval
...
qplot(newStepsEachDay,geom="histogram") + labs(x="Total steps taken each day", 
    y="Frequency count") + geom_histogram(aes(fill = ..count..)) + 
    guides(fill=FALSE) + ggtitle("Frequency of total steps taken each day") +
    theme(plot.title = element_text(lineheight=0.8, face="bold"))
...
The mean total number of steps taken each day using the new dataset is **`r newMeanSteps`**  
The median total number of steps taken each day using the new datatset is  **`r newMedianSteps`**  
...
```{r message=FALSE, warning=FALSE}
# Calculate the average number of steps taken at each 5-min across 
# weekdays & weekends
...
# Time series plot comparing the average number of steps during 
# "weekday" and "weekend" days 
library(ggplot2)
ggplot(data=averageStepsbyWeekday2, aes(x=interval,y=averageSteps, color=day)) +
    geom_line() + facet_grid(day ~. ) + guides(color=FALSE) + 
    labs(x="5-min interval", y="Average number of steps") + 
    ggtitle("Average number of steps at each 5-min interval by day of the week") + 
    theme(plot.title = element_text(lineheight=0.8, face="bold"))
```
